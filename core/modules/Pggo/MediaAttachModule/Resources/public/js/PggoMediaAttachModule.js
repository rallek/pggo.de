...
function pggoMediaAttachCapitaliseFirstLetter(string)
{
 ...
 */
function pggoMediaAttachInitQuickNavigation()
{
    var quickNavForm;
    var objectType;

    if (jQuery('.pggomediaattachmodule-quicknav').length < 1) {
        return;
    }

    quickNavForm = jQuery('.pggomediaattachmodule-quicknav').first();
    objectType = quickNavForm.attr('id').replace('pggoMediaAttachModule', '').replace('QuickNavForm', '');

    quickNavForm.find('select').change(function (event) {
        quickNavForm.submit();
    });

    var fieldPrefix = 'pggomediaattachmodule_' + objectType.toLowerCase() + 'quicknav_';
 ...
 */
function pggoMediaAttachSimpleAlert(beforeElem, title, content, alertId, cssClass)
{
    var alertBox;
 ...
 */
function pggoMediaAttachInitMassToggle()
{
    if (jQuery('.pggomediaattach-mass-toggle').length > 0) {
        jQuery('.pggomediaattach-mass-toggle').click(function (event) {
            jQuery('.pggomediaattach-toggle-checkbox').prop('checked', jQuery(this).prop('checked'));
        });
    }
}
...
jQuery(document).ready(function() {
    ...
    isViewPage = jQuery('.pggomediaattachmodule-view').length > 0;
    isDisplayPage = jQuery('.pggomediaattachmodule-display').length > 0;
