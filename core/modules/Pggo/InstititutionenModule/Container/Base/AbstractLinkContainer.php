...
abstract class AbstractLinkContainer implements LinkContainerInterface
{
    ...
    public function getLinks($type = LinkContainerInterface::TYPE_ADMIN)
    {
        ...
        if (LinkContainerInterface::TYPE_ACCOUNT == $type) {
            ...
            if (true === $this->variableApi->get('PggoInstititutionenModule', 'linkOwnInstitutionsOnAccountPage', true)) {
                $objectType = 'institution';
                if ($this->permissionApi->hasPermission($this->getBundleName() . ':' . ucfirst($objectType) . ':', '::', ACCESS_READ)) {
                    $links[] = [
                        'url' => $this->router->generate('pggoinstititutionenmodule_' . strtolower($objectType) . '_view', ['own' => 1]),
                        'text' => $this->__('My institutions'),
                        'icon' => 'list-alt'
                    ];
                }
            }
        ...
        if (in_array('institution', $allowedObjectTypes)
            && $this->permissionApi->hasPermission($this->getBundleName() . ':Institution:', '::', $permLevel)) {
            $links[] = [
                'url' => $this->router->generate('pggoinstititutionenmodule_institution_' . $routeArea . 'view'),
                'text' => $this->__('Institutions'),
                'title' => $this->__('Institution list')
            ];
        }
        if ($routeArea == 'admin' && $this->permissionApi->hasPermission($this->getBundleName() . '::', '::', ACCESS_ADMIN)) {
            $links[] = [
                'url' => $this->router->generate('pggoinstititutionenmodule_config_config'),
