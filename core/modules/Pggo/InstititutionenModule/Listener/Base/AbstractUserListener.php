...
abstract class AbstractUserListener implements EventSubscriberInterface
{
     ...
     * @return void
     */
    public function __construct(TranslatorInterface $translator, InstititutionenFactory $entityFactory, CurrentUserApi $currentUserApi, LoggerInterface $logger)
    {
    ...
    public function delete(GenericEvent $event)
    {
        ...
        // set creator to admin (2) for all institutions created by this user
        $repo->updateCreator($userId, 2, $this->translator, $this->logger, $this->currentUserApi);
        
        // set last editor to admin (2) for all institutions updated by this user
        $repo->updateLastEditor($userId, 2, $this->translator, $this->logger, $this->currentUserApi);
        
        $logArgs = ['app' => 'PggoInstititutionenModule', 'user' => $this->currentUserApi->get('uname'), 'entities' => 'institutions'];
        $this->logger->notice('{app}: User {user} has been deleted, so we deleted/updated corresponding {entities}, too.', $logArgs);
    }
}
