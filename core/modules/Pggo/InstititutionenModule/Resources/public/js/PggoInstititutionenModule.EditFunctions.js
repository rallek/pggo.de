 ...
 */
function pggoInstititutionenInitUserField(fieldName, getterName)
{
    ...
    jQuery('#' + fieldName + 'Selector').typeahead({
        ...
        source: function (query, syncResults, asyncResults) {
            ...
            jQuery.getJSON(Routing.generate('pggoinstititutionenmodule_ajax_' + getterName.toLowerCase(), { fragment: query }), function( data ) {
 ...
 */
function pggoInstititutionenResetUploadField(fieldName)
{
 ...
 */
function pggoInstititutionenInitUploadField(fieldName)
{
    jQuery('#' + fieldName + 'ResetVal').click( function (event) {
        event.stopPropagation();
        pggoInstititutionenResetUploadField(fieldName);
    }).removeClass('hidden');
}
...
function pggoInstititutionenTriggerFormValidation()
{
    pggoInstititutionenExecuteCustomValidationConstraints(editedObjectType, editedEntityId);
...
function pggoInstititutionenHandleFormSubmit (event) {
    if (triggerValidation) {
        pggoInstititutionenTriggerFormValidation();
        if (!editForm.get(0).checkValidity()) {
 ...
 */
function pggoInstititutionenInitEditForm(mode, entityId)
{
    if (jQuery('.pggoinstititutionen-edit-form').length < 1) {
        return;
    }

    editForm = jQuery('.pggoinstititutionen-edit-form').first();
    ...
    editForm.submit(pggoInstititutionenHandleFormSubmit);
