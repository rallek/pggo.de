...
namespace RK\TeamModule\Entity\Repository\Base;
...
use RK\TeamModule\Helper\ImageHelper;

/**
 * Repository class used to implement own convenience methods for performing certain DQL queries.
 *
 * This is the base repository class for person entities.
 */
abstract class AbstractPersonRepository extends EntityRepository
{
    
    /**
     * @var string The default sorting field/expression
     */
    protected $defaultSortingField = 'lastName';
    ...
    {
        $fieldName = 'teamMemberDescription';
    ...
    public function getAdditionalTemplateParameters(ImageHelper $imageHelper, $context = '', $args = [])
    {
        ...
        if ($context == 'controllerAction') {
            ...
            $objectType = 'person';
            $thumbRuntimeOptions = [];
            $thumbRuntimeOptions[$objectType . 'TeamMemberImage'] = $imageHelper->getRuntimeOptions($objectType, 'teamMemberImage', $context, $args);
            ...
            if (in_array($args['action'], ['display', 'view'])) {
                ...
                $templateParameters['relationThumbRuntimeOptions'] = $imageHelper->getCustomRuntimeOptions('', '', 'RKTeamModule_relateditem', $context, $args);
            }
        }
    ...
    protected function getViewQuickNavParameters($context = '', $args = [])
    {
        ...
        $parameters['registeredUser'] = (int) $this->getRequest()->query->get('registeredUser', 0);
    ...
    public function truncateTable(LoggerInterface $logger)
    {
        ...
        $qb->delete('RK\TeamModule\Entity\PersonEntity', 'tbl');
        $query = $qb->getQuery();
    
        $query->execute();
    
        $logArgs = ['app' => 'RKTeamModule', 'entity' => 'person'];
    ...
    public function updateCreator($userId, $newUserId, TranslatorInterface $translator, LoggerInterface $logger, CurrentUserApi $currentUserApi)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('RK\TeamModule\Entity\PersonEntity', 'tbl')
        ...
        $logArgs = ['app' => 'RKTeamModule', 'user' => $currentUserApi->get('uname'), 'entities' => 'persons', 'userid' => $userId];
    ...
    public function updateUserField($userFieldName, $userId, $newUserId, TranslatorInterface $translator, LoggerInterface $logger, CurrentUserApi $currentUserApi)
    {
        ...
        if (empty($userFieldName) || !in_array($userFieldName, ['registeredUser'])) {
        ...
        $logArgs = ['app' => 'RKTeamModule', 'user' => $currentUserApi->get('uname'), 'entities' => 'persons', 'field' => $userFieldName, 'userid' => $userId, 'newuserid' => $newUserId];
    ...
    public function deleteByUserField($userFieldName, $userId, TranslatorInterface $translator, LoggerInterface $logger, CurrentUserApi $currentUserApi)
    {
        ...
        $logArgs = ['app' => 'RKTeamModule', 'user' => $currentUserApi->get('uname'), 'entities' => 'persons', 'field' => $userFieldName, 'userid' => $userId];
     ...
     *
     * @return array|personEntity retrieved data array or personEntity instance
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false)
     *
     * @return ArrayCollection collection containing retrieved personEntity instances
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
    ...
    protected function applyDefaultFilters(QueryBuilder $qb, $parameters = [])
    {
        ...
        $isAdminArea = false !== strpos($routeName, 'rkteammodule_person_admin');
        if ($isAdminArea) {
            return $qb;
        }
    
        if (!in_array('workflowState', array_keys($parameters)) || empty($parameters['workflowState'])) {
            // per default we show approved persons only
            $onlineStates = ['approved'];
            
            $showOnlyOwnEntries = $this->getRequest()->query->getInt('own', 0);
            if ($showOnlyOwnEntries == 1) {
                // allow the owner to see his deferred persons
    ...
    protected function addSearchFilter(QueryBuilder $qb, $fragment = '')
    {
        ...
        if (!$fragmentIsNumeric) {
            ...
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.lastName LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.firstName LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.teamMemberImage = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.teamMemberFunction LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.teamMemberDescription LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.phoneNumber LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.mobileNumber LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.personEmailAddress = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.personAddress LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.filter LIKE \'%' . $fragment . '%\'';
        } else {
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.workflowState = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            ...
            $where .= 'tbl.personAddress LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.registeredUser = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.filter LIKE \'%' . $fragment . '%\'';
        }
        $where = '(' . $where . ')';
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $selection = 'COUNT(tbl.id) AS numPersons';
        if (true === $useJoins) {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('RK\TeamModule\Entity\PersonEntity', 'tbl');
     ...
     * @param int    $excludeId  Id of persons to exclude (optional)
     *
     * @return boolean result of this check, true if the given person does not already exist
     */
