...
use RK\TeamModule\Helper\ModelHelper;
use RK\TeamModule\Helper\SelectionHelper;
...
abstract class AbstractControllerHelper
{
    ...
    public function processViewActionParameters($objectType, SortableColumns $sortableColumns, array $templateParameters = [], $supportsHooks = false)
    {
        ...
        $showOwnEntries = $request->query->getInt('own', $this->variableApi->get('RKTeamModule', 'showOnlyOwnEntries', 0));
        ...
        if ($showAllEntries != 1) {
            ...
            if (in_array($resultsPerPage, [0, 10])) {
                $resultsPerPage = $this->variableApi->get('RKTeamModule', $objectType . 'EntriesPerPage', 10);
            }
        }
        ...
        $quickNavForm = $this->formFactory->create('RK\TeamModule\Form\Type\QuickNavigation\\' . ucfirst($objectType) . 'QuickNavType', $templateParameters);
        ...
        if (true === $supportsHooks) {
            ...
            $currentUrlObject = new RouteUrl('rkteammodule_' . $objectType . '_' . /*$templateParameters['routeArea'] . */'view', $currentUrlArgs);
        }
    ...
    public function processDisplayActionParameters($objectType, array $templateParameters = [], $supportsHooks = false)
    {
        ...
        if (true === $supportsHooks) {
            ...
            $currentUrlObject = new RouteUrl('rkteammodule_' . $objectType . '_' . /*$templateParameters['routeArea'] . */'display', $currentUrlArgs);
            $templateParameters['currentUrlObject'] = $currentUrlObject;
        }
