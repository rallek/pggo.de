...
<div class="row">
        ...
        {if $properties ne null && is_array($properties)}
            ...
            {foreach key='propertyName' item='propertyId' from=$properties}
                <div class="form-group">
                    ...
                    <div class="col-sm-9">
                        {selector_category name="`$baseID`_`$categorySelectorName``$propertyName`" field='id' selectedValue=$catIds.$propertyName|default:null categoryRegistryModule='PggoInstititutionenModule' categoryRegistryTable="`$objectType`Entity" categoryRegistryProperty=$propertyName defaultText=$lblDefault editLink=false multipleSize=$categorySelectorSize cssClass='form-control'}
                    </div>
                </div>
            {/foreach}
            {/nocache}
        ...
        <div class="form-group">
            ...
            <div class="col-sm-9">
                <select id="{$baseID}Id" name="id" class="form-control">
                    {foreach item='institution' from=$items}
                        <option value="{$institution.id}"{if $selectedId eq $institution.id} selected="selected"{/if}>{$institution->getTitleFromDisplayPattern()}</option>
                    {foreachelse}
            ...
            <div class="col-sm-9">
                <div class="input-group">
                    ...
                    <span class="input-group-btn">
                        <input type="button" id="pggoInstititutionenModuleSearchGo" name="gosearch" value="{gt text='Filter'}" class="btn btn-default" />
                    </span>
                </div>
            </div>
        </div>
    ...
    <div class="col-sm-4">
        <div id="{$baseID}Preview" style="border: 1px dotted #a3a3a3; padding: .2em .5em">
            <p><strong>{gt text='Institution information'}</strong></p>
...
/* <![CDATA[ */
    ( function($) {
        $(document).ready(function() {
            pggoInstititutionenModule.itemSelector.onLoad('{{$baseID}}', {{$selectedId|default:0}});
        });
    })(jQuery);
/* ]]> */
</script>
