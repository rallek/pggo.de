...
namespace Pggo\InstititutionenModule\Form\Handler\Picture\Base;
...
abstract class AbstractEditHandler extends EditHandler
{
    ...
    protected function initRelationPresets()
    {
        ...
        if (!empty($this->relationPresets['institution'])) {
            $relObj = $this->selectionHelper->getEntity('institution', $this->relationPresets['institution']);
            if (null !== $relObj) {
    ...
    protected function createForm()
    {
        ...
        return $this->formFactory->create('Pggo\InstititutionenModule\Form\Type\PictureType', $this->entityRef, $options);
    }
    ...
    protected function getRedirectCodes()
    {
        ...
        // user list of institutions
        $codes[] = 'userViewInstitutions';
        // admin list of institutions
        $codes[] = 'adminViewInstitutions';
        // user list of own institutions
        $codes[] = 'userOwnViewInstitutions';
        // admin list of own institutions
        $codes[] = 'adminOwnViewInstitutions';
        // user detail page of related institution
        $codes[] = 'userDisplayInstitution';
        // admin detail page of related institution
        $codes[] = 'adminDisplayInstitution';
    ...
    public function applyAction(array $args = [])
    {
        ...
        try {
            ...
            $logArgs = ['app' => 'PggoInstititutionenModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => 'picture', 'id' => $entity->createCompositeIdentifier(), 'errorMessage' => $e->getMessage()];
    ...
    protected function getRedirectUrl($args)
    {
        ...
        switch ($this->returnTo) {
            ...
            case 'adminDisplay':
                ...
                return $this->getDefaultReturnUrl($args);
            case 'userViewInstitutions':
            case 'adminViewInstitutions':
                return $this->router->generate('pggoinstititutionenmodule_institution_' . $routeArea . 'view');
            case 'userOwnViewInstitutions':
            case 'adminOwnViewInstitutions':
                return $this->router->generate('pggoinstititutionenmodule_institution_' . $routeArea . 'view', [ 'own' => 1 ]);
            case 'userDisplayInstitution':
            case 'adminDisplayInstitution':
                if (!empty($this->relationPresets['institution'])) {
                    return $this->router->generate('pggoinstititutionenmodule_institution_' . $routeArea . 'display',  ['id' => $this->relationPresets['institution']]);
                }
