 ...
 *
 * This is the base repository class for picture entities.
 */
abstract class AbstractPictureRepository extends EntityRepository
{
    ...
    public function truncateTable(LoggerInterface $logger)
    {
        ...
        $qb->delete('Pggo\InstititutionenModule\Entity\PictureEntity', 'tbl');
        $query = $qb->getQuery();
    
        $query->execute();
    
        $logArgs = ['app' => 'PggoInstititutionenModule', 'entity' => 'picture'];
    ...
    public function updateCreator($userId, $newUserId, TranslatorInterface $translator, LoggerInterface $logger, CurrentUserApi $currentUserApi)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('Pggo\InstititutionenModule\Entity\PictureEntity', 'tbl')
        ...
        $logArgs = ['app' => 'PggoInstititutionenModule', 'user' => $currentUserApi->get('uname'), 'entities' => 'pictures', 'userid' => $userId];
     ...
     *
     * @return array|pictureEntity retrieved data array or pictureEntity instance
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false)
     *
     * @return ArrayCollection collection containing retrieved pictureEntity instances
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
    ...
    protected function applyDefaultFilters(QueryBuilder $qb, $parameters = [])
    {
        ...
        $isAdminArea = false !== strpos($routeName, 'pggoinstititutionenmodule_picture_admin');
        if ($isAdminArea) {
            return $qb;
        }
        ...
        if (!in_array('workflowState', array_keys($parameters)) || empty($parameters['workflowState'])) {
            ...
            if ($showOnlyOwnEntries == 1) {
                // allow the owner to see his deferred pictures
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $selection = 'COUNT(tbl.id) AS numPictures';
        if (true === $useJoins) {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('Pggo\InstititutionenModule\Entity\PictureEntity', 'tbl');
     ...
     * @param int    $excludeId  Id of pictures to exclude (optional)
     *
