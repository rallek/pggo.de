...
abstract class AbstractImageRepository extends EntityRepository
{
    ...
    public function truncateTable(LoggerInterface $logger)
    {
        ...
        $qb->delete('Pggo\InstititutionenModule\Entity\ImageEntity', 'tbl');
        $query = $qb->getQuery();
    
        $query->execute();
    
        $logArgs = ['app' => 'PggoInstititutionenModule', 'entity' => 'image'];
    ...
    public function updateCreator($userId, $newUserId, TranslatorInterface $translator, LoggerInterface $logger, CurrentUserApi $currentUserApi)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('Pggo\InstititutionenModule\Entity\ImageEntity', 'tbl')
        ...
        $logArgs = ['app' => 'PggoInstititutionenModule', 'user' => $currentUserApi->get('uname'), 'entities' => 'images', 'userid' => $userId];
    ...
    protected function applyDefaultFilters(QueryBuilder $qb, $parameters = [])
    {
        ...
        $isAdminArea = false !== strpos($routeName, 'pggoinstititutionenmodule_image_admin');
        if ($isAdminArea) {
            return $qb;
        }
        ...
        if (!in_array('workflowState', array_keys($parameters)) || empty($parameters['workflowState'])) {
            ...
            if ($showOnlyOwnEntries == 1) {
                // allow the owner to see his deferred images
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('Pggo\InstititutionenModule\Entity\ImageEntity', 'tbl');
