 ...
 *
 * This is the base repository class for institution entities.
 */
abstract class AbstractInstitutionRepository extends EntityRepository
{
    ...
    public function getAdditionalTemplateParameters(ImageHelper $imageHelper, $context = '', $args = [])
    {
        ...
        if ($context == 'controllerAction') {
            ...
            if (in_array($args['action'], ['display', 'view'])) {
                ...
                $templateParameters['relationThumbRuntimeOptions'] = $imageHelper->getCustomRuntimeOptions('', '', 'PggoInstititutionenModule_relateditem', $context, $args);
            }
        }
    ...
    public function truncateTable(LoggerInterface $logger)
    {
        ...
        $qb->delete('Pggo\InstititutionenModule\Entity\InstitutionEntity', 'tbl');
        $query = $qb->getQuery();
    
        $query->execute();
    
        $logArgs = ['app' => 'PggoInstititutionenModule', 'entity' => 'institution'];
    ...
    public function updateCreator($userId, $newUserId, TranslatorInterface $translator, LoggerInterface $logger, CurrentUserApi $currentUserApi)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('Pggo\InstititutionenModule\Entity\InstitutionEntity', 'tbl')
        ...
        $logArgs = ['app' => 'PggoInstititutionenModule', 'user' => $currentUserApi->get('uname'), 'entities' => 'institutions', 'userid' => $userId];
     ...
     *
     * @return array|institutionEntity retrieved data array or institutionEntity instance
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false)
     *
     * @return ArrayCollection collection containing retrieved institutionEntity instances
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $selection = 'COUNT(tbl.id) AS numInstitutions';
        if (true === $useJoins) {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('Pggo\InstititutionenModule\Entity\InstitutionEntity', 'tbl');
     ...
     * @param int    $excludeId  Id of institutions to exclude (optional)
     *
     * @return boolean result of this check, true if the given institution does not already exist
     */
