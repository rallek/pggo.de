...
function pggoTeamReadDate(val, includeTime)
{
...
function pggoTeamValidateNoSpace(val)
{
    var valStr;
...
function pggoTeamValidateUploadExtension(val, elem)
{
 ...
 */
function pggoTeamExecuteCustomValidationConstraints(objectType, currentEntityId)
{
    jQuery('.validate-nospace').each( function() {
        if (!pggoTeamValidateNoSpace(jQuery(this).val())) {
    ...
    jQuery('.validate-upload').each( function() {
        if (!pggoTeamValidateUploadExtension(jQuery(this).val(), jQuery(this))) {
