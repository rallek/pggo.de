 ...
 */
function pggoTeamInitUserField(fieldName, getterName)
{
    ...
    jQuery('#' + fieldName + 'Selector').typeahead({
        ...
        source: function (query, syncResults, asyncResults) {
            ...
            jQuery.getJSON(Routing.generate('pggoteammodule_ajax_' + getterName.toLowerCase(), { fragment: query }), function( data ) {
 ...
 */
function pggoTeamResetUploadField(fieldName)
{
 ...
 */
function pggoTeamInitUploadField(fieldName)
{
    jQuery('#' + fieldName + 'ResetVal').click( function (event) {
        event.stopPropagation();
        pggoTeamResetUploadField(fieldName);
    }).removeClass('hidden');
}
...
function pggoTeamTriggerFormValidation()
{
    pggoTeamExecuteCustomValidationConstraints(editedObjectType, editedEntityId);
...
function pggoTeamHandleFormSubmit (event) {
    if (triggerValidation) {
        pggoTeamTriggerFormValidation();
        if (!editForm.get(0).checkValidity()) {
 ...
 */
function pggoTeamInitEditForm(mode, entityId)
{
    if (jQuery('.pggoteam-edit-form').length < 1) {
        return;
    }

    editForm = jQuery('.pggoteam-edit-form').first();
    ...
    editForm.submit(pggoTeamHandleFormSubmit);
