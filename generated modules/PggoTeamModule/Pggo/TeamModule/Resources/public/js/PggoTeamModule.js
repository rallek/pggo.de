...
function pggoTeamCapitaliseFirstLetter(string)
{
 ...
 */
function pggoTeamInitQuickNavigation()
{
    var quickNavForm;
    var objectType;

    if (jQuery('.pggoteammodule-quicknav').length < 1) {
        return;
    }

    quickNavForm = jQuery('.pggoteammodule-quicknav').first();
    objectType = quickNavForm.attr('id').replace('pggoTeamModule', '').replace('QuickNavForm', '');

    quickNavForm.find('select').change(function (event) {
        quickNavForm.submit();
    });

    var fieldPrefix = 'pggoteammodule_' + objectType.toLowerCase() + 'quicknav_';
 ...
 */
function pggoTeamSimpleAlert(beforeElem, title, content, alertId, cssClass)
{
    var alertBox;
 ...
 */
function pggoTeamInitMassToggle()
{
    if (jQuery('.pggoteam-mass-toggle').length > 0) {
        jQuery('.pggoteam-mass-toggle').click(function (event) {
            jQuery('.pggoteam-toggle-checkbox').prop('checked', jQuery(this).prop('checked'));
        });
    }
}
