...
use Pggo\TeamModule\Helper\ImageHelper;
...
abstract class AbstractPersonRepository extends EntityRepository
{
    ...
    public function getAdditionalTemplateParameters(ImageHelper $imageHelper, $context = '', $args = [])
    {
        ...
        if ($context == 'controllerAction') {
            ...
            if (in_array($args['action'], ['display', 'view'])) {
                ...
                $templateParameters['relationThumbRuntimeOptions'] = $imageHelper->getCustomRuntimeOptions('', '', 'PggoTeamModule_relateditem', $context, $args);
            }
        }
    ...
    protected function getViewQuickNavParameters($context = '', $args = [])
    {
        ...
        $parameters = [];
        $categoryHelper = ServiceUtil::get('pggo_team_module.category_helper');
        $parameters['catIdList'] = $categoryHelper->retrieveCategoriesFromRequest('person', 'GET');
    ...
    public function truncateTable(LoggerInterface $logger)
    {
        ...
        $qb->delete('Pggo\TeamModule\Entity\PersonEntity', 'tbl');
        $query = $qb->getQuery();
    
        $query->execute();
    
        $logArgs = ['app' => 'PggoTeamModule', 'entity' => 'person'];
    ...
    public function updateCreator($userId, $newUserId, TranslatorInterface $translator, LoggerInterface $logger, CurrentUserApi $currentUserApi)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('Pggo\TeamModule\Entity\PersonEntity', 'tbl')
        ...
        $logArgs = ['app' => 'PggoTeamModule', 'user' => $currentUserApi->get('uname'), 'entities' => 'persons', 'userid' => $userId];
    ...
    public function addCommonViewFilters(QueryBuilder $qb)
    {
        ...
        foreach ($parameters as $k => $v) {
            if ($k == 'catId') {
                 ...
                 */
                $categoryHelper = ServiceUtil::get('pggo_team_module.category_helper');
                $qb = $categoryHelper->buildFilterClauses($qb, 'person', $v);
    ...
    protected function addSearchFilter(QueryBuilder $qb, $fragment = '')
    {
        ...
        if (!$fragmentIsNumeric) {
            ...
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.functionRole LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.emailAddress = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.contact LIKE \'%' . $fragment . '%\'';
        } else {
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.lastName LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('Pggo\TeamModule\Entity\PersonEntity', 'tbl');
    ...
    protected function genericBaseQueryAddWhere(QueryBuilder $qb, $where = '')
    {
        if (!empty($where)) {
            ...
            $categoryHelper = ServiceUtil::get('pggo_team_module.category_helper');
            $categoryProperties = $categoryHelper->getAllProperties('person');
            foreach ($categoryProperties as $propertyName => $registryId) {
                $config['plugins'][] = new CategoryFilter('PggoTeamModule', $propertyName, 'categories' . ucfirst($propertyName));
            }
