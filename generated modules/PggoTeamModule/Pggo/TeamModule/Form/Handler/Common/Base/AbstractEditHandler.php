...
namespace Pggo\TeamModule\Form\Handler\Common\Base;
...
use Pggo\TeamModule\Helper\HookHelper;
use Pggo\TeamModule\Helper\ModelHelper;
use Pggo\TeamModule\Helper\SelectionHelper;
...
abstract class AbstractEditHandler
{
     ...
     * @param TeamFactory $entityFactory TeamFactory service instance
    ...
    public function processForm(array $templateParameters)
    {
        ...
        if (null === $this->returnTo) {
            // default to referer
            if ($this->request->getSession()->has('pggoteammoduleReferer')) {
                $this->returnTo = $this->request->getSession()->get('pggoteammoduleReferer');
            } elseif ($this->request->headers->has('pggoteammoduleReferer')) {
                $this->returnTo = $this->request->headers->get('pggoteammoduleReferer');
                $this->request->getSession()->set('pggoteammoduleReferer', $this->returnTo);
        ...
        $this->permissionComponent = 'PggoTeamModule:' . $this->objectTypeCapital . ':';
        ...
        if ($this->templateParameters['mode'] == 'edit') {
            ...
            if (null !== $entity) {
                if (true === $this->hasPageLockSupport && $this->kernel->isBundle('ZikulaPageLockModule') && null !== $this->lockingApi) {
                    ...
                    $lockName = 'PggoTeamModule' . $this->objectTypeCapital . $this->createCompositeIdentifier();
        ...
        if (false === $actions || !is_array($actions)) {
            ...
            $logArgs = ['app' => 'PggoTeamModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType, 'id' => $entity->createCompositeIdentifier()];
        ...
        if ($isRegularAction || $action == 'delete') {
            ...
            if ($entity->supportsHookSubscribers()) {
                ...
                if ($action != 'delete') {
                    ...
                    $url = new RouteUrl('pggoteammodule_' . $this->objectType . '_display', $urlArgs);
                }
    ...
    protected function addDefaultMessage($args, $success = false)
    {
        ...
        $logArgs = ['app' => 'PggoTeamModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType, 'id' => $this->entityRef->createCompositeIdentifier()];
        if (true === $success) {
    ...
    public function fetchInputData($args)
    {
        ...
        if (isset($this->form['additionalNotificationRemarks']) && $this->form['additionalNotificationRemarks']->getData() != '') {
            $this->request->getSession()->set('PggoTeamModuleAdditionalNotificationRemarks', $this->form['additionalNotificationRemarks']->getData());
        }
