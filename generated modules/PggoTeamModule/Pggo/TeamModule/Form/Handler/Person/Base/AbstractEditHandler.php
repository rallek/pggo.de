...
namespace Pggo\TeamModule\Form\Handler\Person\Base;

use Pggo\TeamModule\Form\Handler\Common\EditHandler;
...
abstract class AbstractEditHandler extends EditHandler
{
    ...
    public function processForm(array $templateParameters)
    {
        ...
        $this->objectTypeCapital = 'Person';
        $this->objectTypeLower = 'person';
        ...
        if ($this->templateParameters['mode'] == 'create') {
            if (!$this->modelHelper->canBeCreated($this->objectType)) {
                $this->request->getSession()->getFlashBag()->add('error', $this->__('Sorry, but you can not create the person yet as other items are required which must be created before!'));
                $logArgs = ['app' => 'PggoTeamModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType];
    ...
    protected function createForm()
    {
        ...
        return $this->formFactory->create('Pggo\TeamModule\Form\Type\PersonType', $this->entityRef, $options);
    }
    ...
    protected function getRedirectCodes()
    {
        ...
        // user index page of person area
        $codes[] = 'userIndex';
        // admin index page of person area
        $codes[] = 'adminIndex';
        // user list of persons
        $codes[] = 'userView';
        // admin list of persons
        $codes[] = 'adminView';
        // user list of own persons
        $codes[] = 'userOwnView';
        // admin list of own persons
        $codes[] = 'adminOwnView';
        // user detail page of treated person
        $codes[] = 'userDisplay';
        // admin detail page of treated person
        $codes[] = 'adminDisplay';
    ...
    protected function getDefaultReturnUrl($args)
    {
        ...
        $routePrefix = 'pggoteammodule_' . $this->objectTypeLower . '_' . $routeArea;
    ...
    protected function getDefaultMessage($args, $success = false)
    {
        ...
        switch ($args['commandName']) {
            case 'submit':
                if ($this->templateParameters['mode'] == 'create') {
                    $message = $this->__('Done! Person created.');
                } else {
                    $message = $this->__('Done! Person updated.');
                }
                break;
            case 'delete':
                $message = $this->__('Done! Person deleted.');
                break;
            default:
                $message = $this->__('Done! Person updated.');
                break;
    ...
    public function applyAction(array $args = [])
    {
        ...
        try {
            ...
            $logArgs = ['app' => 'PggoTeamModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => 'person', 'id' => $entity->createCompositeIdentifier(), 'errorMessage' => $e->getMessage()];
    ...
    protected function getRedirectUrl($args)
    {
        ...
        if ($this->request->getSession()->has('pggoteammoduleReferer')) {
            $this->request->getSession()->del('pggoteammoduleReferer');
        }
