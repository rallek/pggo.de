...
namespace Pggo\MediaAttachModule\Form\Handler\Common\Base;
...
use Pggo\MediaAttachModule\Helper\HookHelper;
use Pggo\MediaAttachModule\Helper\ModelHelper;
use Pggo\MediaAttachModule\Helper\SelectionHelper;
...
abstract class AbstractEditHandler
{
    ...
    public function processForm(array $templateParameters)
    {
        ...
        if (null === $this->returnTo) {
            // default to referer
            if ($this->request->getSession()->has('pggomediaattachmoduleReferer')) {
                $this->returnTo = $this->request->getSession()->get('pggomediaattachmoduleReferer');
            } elseif ($this->request->headers->has('pggomediaattachmoduleReferer')) {
                $this->returnTo = $this->request->headers->get('pggomediaattachmoduleReferer');
                $this->request->getSession()->set('pggomediaattachmoduleReferer', $this->returnTo);
        ...
        $this->permissionComponent = 'PggoMediaAttachModule:' . $this->objectTypeCapital . ':';
        ...
        if ($this->templateParameters['mode'] == 'edit') {
            ...
            if (null !== $entity) {
                if (true === $this->hasPageLockSupport && $this->kernel->isBundle('ZikulaPageLockModule') && null !== $this->lockingApi) {
                    ...
                    $lockName = 'PggoMediaAttachModule' . $this->objectTypeCapital . $this->createCompositeIdentifier();
        ...
        if (false === $actions || !is_array($actions)) {
            ...
            $logArgs = ['app' => 'PggoMediaAttachModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType, 'id' => $entity->createCompositeIdentifier()];
        ...
        if ($isRegularAction || $action == 'delete') {
            ...
            if ($entity->supportsHookSubscribers()) {
                ...
                if ($action != 'delete') {
                    ...
                    $url = new RouteUrl('pggomediaattachmodule_' . $this->objectType . '_display', $urlArgs);
                }
    ...
    protected function addDefaultMessage($args, $success = false)
    {
        ...
        $logArgs = ['app' => 'PggoMediaAttachModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType, 'id' => $this->entityRef->createCompositeIdentifier()];
        if (true === $success) {
    ...
    public function fetchInputData($args)
    {
        ...
        if (isset($this->form['additionalNotificationRemarks']) && $this->form['additionalNotificationRemarks']->getData() != '') {
            $this->request->getSession()->set('PggoMediaAttachModuleAdditionalNotificationRemarks', $this->form['additionalNotificationRemarks']->getData());
        }
