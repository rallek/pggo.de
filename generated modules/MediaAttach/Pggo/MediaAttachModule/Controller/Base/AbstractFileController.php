...
abstract class AbstractFileController extends AbstractController
{
    ...
    protected function indexInternal(Request $request, $isAdmin = false)
    {
        ...
        return $this->redirectToRoute('pggomediaattachmodule_file_' . $templateParameters['routeArea'] . 'view');
        
        // return index template
        return $this->render('@PggoMediaAttachModule/File/index.html.twig', $templateParameters);
    }
    /**
    ...
    protected function viewInternal(Request $request, $sort, $sortdir, $pos, $num, $isAdmin = false)
    {
        ...
        $viewHelper = $this->get('pggo_mediaattach_module.view_helper');
        ...
        $sortableColumns = new SortableColumns($this->get('router'), 'pggomediaattachmodule_file_' . ($isAdmin ? 'admin' : '') . 'view', 'sort', 'sortdir');
     ...
     * @ParamConverter("file", class="PggoMediaAttachModule:FileEntity", options = {"id" = "id", "repository_method" = "selectById"})
    ...
    protected function displayInternal(Request $request, FileEntity $file, $isAdmin = false)
    {
        ...
        // fetch and return the appropriate template
        $response = $this->get('pggo_mediaattach_module.view_helper')->processTemplate($objectType, 'display', $templateParameters);
    ...
    protected function editInternal(Request $request, $isAdmin = false)
    {
        ...
        $formHandler = $this->get('pggo_mediaattach_module.form.handler.file');
        ...
        // fetch and return the appropriate template
        return $this->get('pggo_mediaattach_module.view_helper')->processTemplate($objectType, 'edit', $templateParameters);
    }
    /**
    ...
    protected function deleteInternal(Request $request, FileEntity $file, $isAdmin = false)
    {
        ...
        $logArgs = ['app' => 'PggoMediaAttachModule', 'user' => $this->get('zikula_users_module.current_user')->get('uname'), 'entity' => 'file', 'id' => $file->createCompositeIdentifier()];
        
        $file->initWorkflow();
        
        // determine available workflow actions
        $workflowHelper = $this->get('pggo_mediaattach_module.workflow_helper');
        ...
        $redirectRoute = 'pggomediaattachmodule_file_' . ($isAdmin ? 'admin' : '') . 'view';
        ...
        $form = $this->createForm('Pggo\MediaAttachModule\Form\DeleteEntityType', $file);
        
        if ($form->handleRequest($request)->isValid()) {
            if ($form->get('delete')->isClicked()) {
                $hookHelper = $this->get('pggo_mediaattach_module.hook_helper');
        ...
        // fetch and return the appropriate template
        return $this->get('pggo_mediaattach_module.view_helper')->processTemplate($objectType, 'delete', $templateParameters);
    }
    ...
    protected function handleSelectedEntriesActionInternal(Request $request, $isAdmin = false)
    {
        ...
        foreach ($items as $itemid) {
            ...
            try {
                ...
                $logger->error('{app}: User {user} tried to execute the {action} workflow action for the {entity} with id {id}, but failed. Error details: {errorMessage}.', ['app' => 'PggoMediaAttachModule', 'user' => $userName, 'action' => $action, 'entity' => 'file', 'id' => $itemid, 'errorMessage' => $e->getMessage()]);
            }
            ...
            if ($action == 'delete') {
                $this->addFlash('status', $this->__('Done! Item deleted.'));
                $logger->notice('{app}: User {user} deleted the {entity} with id {id}.', ['app' => 'PggoMediaAttachModule', 'user' => $userName, 'entity' => 'file', 'id' => $itemid]);
            } else {
                $this->addFlash('status', $this->__('Done! Item updated.'));
                $logger->notice('{app}: User {user} executed the {action} workflow action for the {entity} with id {id}.', ['app' => 'PggoMediaAttachModule', 'user' => $userName, 'action' => $action, 'entity' => 'file', 'id' => $itemid]);
            }
            ...
            if ($action != 'delete') {
                ...
                $url = new RouteUrl('pggomediaattachmodule_file_' . /*($isAdmin ? 'admin' : '') . */'display', $urlArgs);
            }
            $hookHelper->callProcessHooks($entity, $hookType, $url);
        }
        
        return $this->redirectToRoute('pggomediaattachmodule_file_' . ($isAdmin ? 'admin' : '') . 'index');
    }
}
