 ...
 */
abstract class AbstractEventController extends AbstractController
{
    /**
    ...
    protected function indexInternal(Request $request, $isAdmin = false)
    {
        ...
        return $this->redirectToRoute('pggonewsdatesmodule_event_' . $templateParameters['routeArea'] . 'view');
        
        // return index template
        return $this->render('@PggoNewsDatesModule/Event/index.html.twig', $templateParameters);
    }
    /**
    ...
    protected function viewInternal(Request $request, $sort, $sortdir, $pos, $num, $isAdmin = false)
    {
        ...
        $sortableColumns = new SortableColumns($this->get('router'), 'pggonewsdatesmodule_event_' . ($isAdmin ? 'admin' : '') . 'view', 'sort', 'sortdir');
    ...
    {
        return $this->displayInternal($request, $event, true);
    }
    ...
    {
        return $this->displayInternal($request, $event, false);
    }
    
    /**
     * This method includes the common implementation code for adminDisplay() and display().
     */
    protected function displayInternal(Request $request, EventEntity $event, $isAdmin = false)
    {
        // parameter specifying which type of objects we are treating
        $objectType = 'event';
        $permLevel = $isAdmin ? ACCESS_ADMIN : ACCESS_READ;
        ...
        $featureActivationHelper = $this->get('pggo_newsdates_module.feature_activation_helper');
        if ($featureActivationHelper->isEnabled(FeatureActivationHelper::CATEGORIES, $objectType)) {
            if (!$this->get('pggo_newsdates_module.category_helper')->hasPermission($event)) {
                throw new AccessDeniedException();
            }
        }
    ...
    protected function editInternal(Request $request, $isAdmin = false)
    {
        ...
        $formHandler = $this->get('pggo_newsdates_module.form.handler.event');
    ...
    {
        return $this->deleteInternal($request, $event, true);
    }
    ...
    {
        return $this->deleteInternal($request, $event, false);
    }
    
    /**
     * This method includes the common implementation code for adminDelete() and delete().
     */
    protected function deleteInternal(Request $request, EventEntity $event, $isAdmin = false)
    {
        // parameter specifying which type of objects we are treating
        $objectType = 'event';
        ...
        $logArgs = ['app' => 'PggoNewsDatesModule', 'user' => $this->get('zikula_users_module.current_user')->get('uname'), 'entity' => 'event', 'id' => $event->createCompositeIdentifier()];
        
        $event->initWorkflow();
        
        // determine available workflow actions
        $workflowHelper = $this->get('pggo_newsdates_module.workflow_helper');
        $actions = $workflowHelper->getActionsForObject($event);
        ...
        $redirectRoute = 'pggonewsdatesmodule_event_' . ($isAdmin ? 'admin' : '') . 'view';
        ...
        if (!$deleteAllowed) {
            $this->addFlash('error', $this->__('Error! It is not allowed to delete this event.'));
            $logger->error('{app}: User {user} tried to delete the {entity} with id {id}, but this action was not allowed.', $logArgs);
        
            return $this->redirectToRoute($redirectRoute);
        }
        
        $form = $this->createForm('Pggo\NewsDatesModule\Form\DeleteEntityType', $event);
        
        if ($form->handleRequest($request)->isValid()) {
            if ($form->get('delete')->isClicked()) {
                $hookHelper = $this->get('pggo_newsdates_module.hook_helper');
                // Let any hooks perform additional validation actions
                $validationHooksPassed = $hookHelper->callValidationHooks($event, 'validate_delete');
                if ($validationHooksPassed) {
                    // execute the workflow action
                    $success = $workflowHelper->executeAction($event, $deleteActionId);
                    if ($success) {
                        $this->addFlash('status', $this->__('Done! Item deleted.'));
                        $logger->notice('{app}: User {user} deleted the {entity} with id {id}.', $logArgs);
                    }
                    
                    // Let any hooks know that we have deleted the event
                    $hookHelper->callProcessHooks($event, 'process_delete', null);
    ...
    protected function handleSelectedEntriesActionInternal(Request $request, $isAdmin = false)
    {
        ...
        foreach ($items as $itemid) {
            ...
            try {
                ...
                $logger->error('{app}: User {user} tried to execute the {action} workflow action for the {entity} with id {id}, but failed. Error details: {errorMessage}.', ['app' => 'PggoNewsDatesModule', 'user' => $userName, 'action' => $action, 'entity' => 'event', 'id' => $itemid, 'errorMessage' => $e->getMessage()]);
            }
            ...
            if ($action == 'delete') {
                $this->addFlash('status', $this->__('Done! Item deleted.'));
                $logger->notice('{app}: User {user} deleted the {entity} with id {id}.', ['app' => 'PggoNewsDatesModule', 'user' => $userName, 'entity' => 'event', 'id' => $itemid]);
            } else {
                $this->addFlash('status', $this->__('Done! Item updated.'));
                $logger->notice('{app}: User {user} executed the {action} workflow action for the {entity} with id {id}.', ['app' => 'PggoNewsDatesModule', 'user' => $userName, 'action' => $action, 'entity' => 'event', 'id' => $itemid]);
            }
            ...
            if ($action != 'delete') {
                ...
                $url = new RouteUrl('pggonewsdatesmodule_event_' . /*($isAdmin ? 'admin' : '') . */'display', $urlArgs);
            }
            $hookHelper->callProcessHooks($entity, $hookType, $url);
        }
        
        return $this->redirectToRoute('pggonewsdatesmodule_event_' . ($isAdmin ? 'admin' : '') . 'index');
    }
}
