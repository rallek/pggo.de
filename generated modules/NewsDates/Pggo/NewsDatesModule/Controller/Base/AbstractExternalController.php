...
abstract class AbstractExternalController extends AbstractController
{
    ...
    public function displayAction($objectType, $id, $source, $displayMode)
    {
        ...
        $templateParameters['featureActivationHelper'] = $this->get('pggo_newsdates_module.feature_activation_helper');
        
        return $this->render('@PggoNewsDatesModule/External/' . ucfirst($objectType) . '/display.html.twig', $templateParameters);
    }
    ...
    public function finderAction(Request $request, $objectType, $editor, $sort, $sortdir, $pos = 1, $num = 0)
    {
        ...
        if (!$this->hasPermission('PggoNewsDatesModule:' . ucfirst($objectType) . ':', '::', ACCESS_COMMENT)) {
        ...
        $form = $this->createForm('Pggo\NewsDatesModule\Form\Type\Finder\\' . ucfirst($objectType) . 'FinderType', $templateParameters, $formOptions);
        ...
        if (in_array($objectType, ['article', 'event'])) {
            $featureActivationHelper = $this->get('pggo_newsdates_module.feature_activation_helper');
            if ($featureActivationHelper->isEnabled(FeatureActivationHelper::CATEGORIES, $objectType)) {
                $entities = $this->get('pggo_newsdates_module.category_helper')->filterEntitiesByPermission($entities);
            }
        }
        ...
        $imageHelper = $this->get('pggo_newsdates_module.image_helper');
        ...
        $output = $this->renderView('@PggoNewsDatesModule/External/' . ucfirst($objectType) . '/find.html.twig', $templateParameters);
