...
abstract class AbstractUploadHelper
{
        ...
        $dataDirectory)
    {
        ...
        $this->allowedObjectTypes = ['article', 'picture'];
    ...
    public function performFileUpload($objectType, $file, $fieldName)
    {
        ...
        try {
            ...
            $this->logger->error('{app}: User {user} could not detect upload destination path for entity {entity} and field {field}.', ['app' => 'PggoNewsDatesModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $objectType, 'field' => $fieldName]);
        ...
        if ($isImage) {
            ...
            if (!is_array($imgInfo) || !$imgInfo[0] || !$imgInfo[1]) {
                ...
                $this->logger->error('{app}: User {user} tried to upload a file which is seems not to be a valid image.', ['app' => 'PggoNewsDatesModule', 'user' => $this->currentUserApi->get('uname')]);
        ...
        if ($isImage) {
            ...
            if (true === $this->variableApi->get('PggoNewsDatesModule', 'enableShrinkingFor' . $fieldSuffix, false)) {
                // check for maximum size
                $maxWidth = $this->variableApi->get('PggoNewsDatesModule', 'shrinkWidth' . $fieldSuffix, 800);
                $maxHeight = $this->variableApi->get('PggoNewsDatesModule', 'shrinkHeight' . $fieldSuffix, 600);
                ...
                if ($imgInfo[0] > $maxWidth || $imgInfo[1] > $maxHeight) {
                    ...
                    $preset = new \SystemPlugin_Imagine_Preset('PggoNewsDatesModule_Shrinker', [
                        'width' => $maxWidth,
                        'height' => $maxHeight,
                        'mode' => 'inset'
                    ]);
    ...
    protected function validateFileUpload($objectType, $file, $fieldName)
    {
        ...
        if ($file->getError() != UPLOAD_ERR_OK) {
            ...
            $this->logger->error('{app}: User {user} tried to upload a file with errors: ' . $file->getErrorMessage(), ['app' => 'PggoNewsDatesModule', 'user' => $this->currentUserApi->get('uname')]);
        ...
        if (false === $isValidExtension) {
            ...
            $this->logger->error('{app}: User {user} tried to upload a file with a forbidden extension ("{extension}").', ['app' => 'PggoNewsDatesModule', 'user' => $this->currentUserApi->get('uname'), 'extension' => $extension]);
    ...
    {
        $basePath = $this->dataDirectory . '/PggoNewsDatesModule/';
    
        switch ($objectType) {
            case 'article':
                $basePath .= 'articles/image/';
                break;
            case 'picture':
    ...
    public function checkAndCreateAllUploadFolders()
    {
        ...
        $result &= $this->checkAndCreateUploadFolder('article', 'image', 'gif, jpeg, jpg, png');
    ...
    protected function checkAndCreateUploadFolder($objectType, $fieldName, $allowedExtensions = '')
    {
        ...
        if (!$fs->exists($uploadPath)) {
            try {
                ...
                $this->logger->error('{app}: The upload directory {directory} does not exist and could not be created.', ['app' => 'PggoNewsDatesModule', 'directory' => $uploadPath]);
        ...
        if (!is_writable($uploadPath)) {
            try {
                ...
                $this->logger->error('{app}: The upload directory {directory} exists but is not writable by the webserver.', ['app' => 'PggoNewsDatesModule', 'directory' => $uploadPath]);
        ...
        $htaccessFileTemplate = 'modules/Pggo/NewsDatesModule/Resources/docs/htaccessTemplate';
        if (!$fs->exists($htaccessFilePath) && $fs->exists($htaccessFileTemplate)) {
            try {
                ...
                $this->logger->error('{app}: An error occured during creation of the .htaccess file in directory {directory}.', ['app' => 'PggoNewsDatesModule', 'directory' => $uploadPath]);
            }
        }
