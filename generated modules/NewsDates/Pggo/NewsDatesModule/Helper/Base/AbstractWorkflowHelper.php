...
abstract class AbstractWorkflowHelper
{
     ...
     * @return void
     */
    public function __construct(TranslatorInterface $translator, LoggerInterface $logger, PermissionApi $permissionApi, NewsDatesFactory $entityFactory, ListEntriesHelper $listEntriesHelper)
    {
        $this->name = 'PggoNewsDatesModule';
    ...
    public function executeAction($entity, $actionId = '', $recursive = false)
    {
        ...
        $result = Zikula_Workflow_Util::executeAction($schemaName, $entity, $actionId, $objectType, 'PggoNewsDatesModule', $idColumn);
    ...
    public function normaliseWorkflowData(&$entity)
    {
        ...
        if (!is_object($workflow)) {
            $workflow['module'] = 'PggoNewsDatesModule';
    ...
    public function collectAmountOfModerationItems()
    {
        ...
        $state = 'waiting';
        $objectType = 'article';
        if ($this->permissionApi->hasPermission('PggoNewsDatesModule:' . ucfirst($objectType) . ':', '::', ACCESS_ADD)) {
            $amount = $this->getAmountOfModerationItems($objectType, $state);
            if ($amount > 0) {
                $amounts[] = [
                    'aggregateType' => 'articlesApproval',
                    'description' => $this->translator->__('Articles pending approval'),
                    'amount' => $amount,
