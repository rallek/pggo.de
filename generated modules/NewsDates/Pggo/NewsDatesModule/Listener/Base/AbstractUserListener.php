...
abstract class AbstractUserListener implements EventSubscriberInterface
{
     ...
     * @return void
     */
    public function __construct(TranslatorInterface $translator, NewsDatesFactory $entityFactory, CurrentUserApi $currentUserApi, LoggerInterface $logger)
    {
    ...
    public function delete(GenericEvent $event)
    {
        ...
        $repo = $this->entityFactory->getRepository('article');
        // set creator to admin (2) for all articles created by this user
        $repo->updateCreator($userId, 2, $this->translator, $this->logger, $this->currentUserApi);
        
        // set last editor to admin (2) for all articles updated by this user
        $repo->updateLastEditor($userId, 2, $this->translator, $this->logger, $this->currentUserApi);
        
        $logArgs = ['app' => 'PggoNewsDatesModule', 'user' => $this->currentUserApi->get('uname'), 'entities' => 'articles'];
        ...
        $logArgs = ['app' => 'PggoNewsDatesModule', 'user' => $this->currentUserApi->get('uname'), 'entities' => 'events'];
        $this->logger->notice('{app}: User {user} has been deleted, so we deleted/updated corresponding {entities}, too.', $logArgs);
    }
}
