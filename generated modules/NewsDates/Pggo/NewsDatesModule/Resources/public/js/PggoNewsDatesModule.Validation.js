...
function pggoNewsDatesToday(format)
{
...
function pggoNewsDatesReadDate(val, includeTime)
{
...
function pggoNewsDatesValidateNoSpace(val)
{
    var valStr;
...
function pggoNewsDatesValidateUploadExtension(val, elem)
{
...
function pggoNewsDatesValidateDateRangeArticle(val)
{
 ...
 */
function pggoNewsDatesExecuteCustomValidationConstraints(objectType, currentEntityId)
{
    jQuery('.validate-nospace').each( function() {
        if (!pggoNewsDatesValidateNoSpace(jQuery(this).val())) {
    ...
    jQuery('.validate-upload').each( function() {
        if (!pggoNewsDatesValidateUploadExtension(jQuery(this).val(), jQuery(this))) {
            document.getElementById(jQuery(this).attr('id')).setCustomValidity(Translator.__('Please select a valid file extension.'));
        } else {
            document.getElementById(jQuery(this).attr('id')).setCustomValidity('');
        }
    });
    jQuery('.validate-daterange-article').each( function() {
        if (typeof jQuery(this).attr('id') != 'undefined') {
        if (!pggoNewsDatesValidateDateRangeArticle(jQuery(this).val())) {
