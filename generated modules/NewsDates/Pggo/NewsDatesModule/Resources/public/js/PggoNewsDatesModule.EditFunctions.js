 ...
 */
function pggoNewsDatesInitUserField(fieldName, getterName)
{
    ...
    jQuery('#' + fieldName + 'Selector').typeahead({
        ...
        source: function (query, syncResults, asyncResults) {
            ...
            jQuery.getJSON(Routing.generate('pggonewsdatesmodule_ajax_' + getterName.toLowerCase(), { fragment: query }), function( data ) {
 ...
 */
function pggoNewsDatesResetUploadField(fieldName)
{
 ...
 */
function pggoNewsDatesInitUploadField(fieldName)
{
    jQuery('#' + fieldName + 'ResetVal').click( function (event) {
        event.stopPropagation();
        pggoNewsDatesResetUploadField(fieldName);
    }).removeClass('hidden');
}
 ...
 */
function pggoNewsDatesResetDateField(fieldName)
{
 ...
 */
function pggoNewsDatesInitDateField(fieldName)
{
    jQuery('#' + fieldName + 'ResetVal').click( function (event) {
        event.stopPropagation();
        pggoNewsDatesResetDateField(fieldName);
    }).removeClass('hidden');
}
...
function pggoNewsDatesTriggerFormValidation()
{
    pggoNewsDatesExecuteCustomValidationConstraints(editedObjectType, editedEntityId);
...
function pggoNewsDatesHandleFormSubmit (event) {
    if (triggerValidation) {
        pggoNewsDatesTriggerFormValidation();
        if (!editForm.get(0).checkValidity()) {
 ...
 */
function pggoNewsDatesInitEditForm(mode, entityId)
{
    if (jQuery('.pggonewsdates-edit-form').length < 1) {
        return;
    }

    editForm = jQuery('.pggonewsdates-edit-form').first();
    ...
    editForm.submit(pggoNewsDatesHandleFormSubmit);
