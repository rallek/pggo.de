...
namespace Pggo\NewsDatesModule\ContentType\Base;
...
abstract class AbstractItemList extends \Content_AbstractContentType implements ContainerAwareInterface
{
    ...
    {
        return $this->container->get('translator.default')->__('PggoNewsDatesModule list view');
    }
    ...
    {
        return $this->container->get('translator.default')->__('Display list of PggoNewsDatesModule objects.');
    }
    ...
    {
        $controllerHelper = $this->container->get('pggo_newsdates_module.controller_helper');
        ...
        $featureActivationHelper = $this->container->get('pggo_newsdates_module.feature_activation_helper');
        if ($featureActivationHelper->isEnabled(FeatureActivationHelper::CATEGORIES, $this->objectType)) {
            $this->categorisableObjectTypes = ['article', 'event'];
            $categoryHelper = $this->container->get('pggo_newsdates_module.category_helper');
    
            // fetch category properties
            $this->catRegistries = [];
            $this->catProperties = [];
            if (in_array($this->objectType, $this->categorisableObjectTypes)) {
                $selectionHelper = $this->container->get('pggo_newsdates_module.selection_helper');
    ...
    {
        $repository = $this->container->get('pggo_newsdates_module.entity_factory')->getRepository($this->objectType);
        ...
        $imageHelper = $this->container->get('pggo_newsdates_module.image_helper');
    ...
    protected function getDisplayTemplate()
    {
        ...
        foreach ($templateOptions as $templatePath) {
            if ($templating->exists('@PggoNewsDatesModule/' . $templatePath)) {
                $template = '@PggoNewsDatesModule/' . $templatePath;
                break;
            }
        }
    ...
    public function startEditing()
    {
        ...
        $this->view->toplevelmodule = 'PggoNewsDatesModule';
    
        // ensure our custom plugins are loaded
        array_push($this->view->plugins_dir, 'modules/Pggo/NewsDatesModule/Resources/views//plugins');
        ...
        if ($featureActivationHelper->isEnabled(FeatureActivationHelper::CATEGORIES, $this->objectType)) {
            ...
            $this->view->assign('categories', $categories)
                       ->assign('categoryHelper', $this->container->get('pggo_newsdates_module.category_helper'));
        }
    }
