...
namespace Pggo\NewsDatesModule\Form\Handler\Common\Base;
...
use Pggo\NewsDatesModule\Helper\ModelHelper;
use Pggo\NewsDatesModule\Helper\SelectionHelper;
...
abstract class AbstractEditHandler
{
    ...
    public function processForm(array $templateParameters)
    {
        ...
        if (null === $this->returnTo) {
            ...
            if ($this->request->getSession()->has('pggonewsdatesmoduleReferer')) {
                $this->returnTo = $this->request->getSession()->get('pggonewsdatesmoduleReferer');
            } elseif ($this->request->headers->has('pggonewsdatesmoduleReferer')) {
                $this->returnTo = $this->request->headers->get('pggonewsdatesmoduleReferer');
                $this->request->getSession()->set('pggonewsdatesmoduleReferer', $this->returnTo);
        ...
        $this->permissionComponent = 'PggoNewsDatesModule:' . $this->objectTypeCapital . ':';
        ...
        if ($this->templateParameters['mode'] == 'edit') {
            ...
            if (null !== $entity) {
                if (true === $this->hasPageLockSupport && $this->kernel->isBundle('ZikulaPageLockModule') && null !== $this->lockingApi) {
                    ...
                    $lockName = 'PggoNewsDatesModule' . $this->objectTypeCapital . $this->createCompositeIdentifier();
        ...
        if (false === $actions || !is_array($actions)) {
            ...
            $logArgs = ['app' => 'PggoNewsDatesModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType, 'id' => $entity->createCompositeIdentifier()];
        ...
        if ($isRegularAction || $action == 'delete') {
            ...
            if ($entity->supportsHookSubscribers()) {
                ...
                if ($action != 'delete') {
                    ...
                    $url = new RouteUrl('pggonewsdatesmodule_' . $this->objectType . '_display', $urlArgs);
                }
    ...
    protected function addDefaultMessage($args, $success = false)
    {
        ...
        $logArgs = ['app' => 'PggoNewsDatesModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType, 'id' => $this->entityRef->createCompositeIdentifier()];
        if (true === $success) {
    ...
    public function fetchInputData($args)
    {
        ...
        if (isset($this->form['additionalNotificationRemarks']) && $this->form['additionalNotificationRemarks']->getData() != '') {
            $this->request->getSession()->set('PggoNewsDatesModuleAdditionalNotificationRemarks', $this->form['additionalNotificationRemarks']->getData());
        }
    ...
    protected function prepareWorkflowAdditions($enterprise = false)
    {
        ...
        $groupApplicationArgs = [
            'user' => $currentUserId,
            'group' => $this->variableApi->get('PggoNewsDatesModule', 'moderationGroupFor' . $this->objectTypeCapital, 2)
        ];
        ...
        if (true === $enterprise) {
            $groupApplicationArgs = [
                'user' => $currentUserId,
                'group' => $this->variableApi->get('PggoNewsDatesModule', 'superModerationGroupFor' . $this->objectTypeCapital, 2)
            ];
