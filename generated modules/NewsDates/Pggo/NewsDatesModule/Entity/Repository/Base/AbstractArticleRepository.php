...
use Pggo\NewsDatesModule\Helper\ImageHelper;
use Pggo\NewsDatesModule\Helper\WorkflowHelper;

/**
 * Repository class used to implement own convenience methods for performing certain DQL queries.
 *
 * This is the base repository class for article entities.
 */
abstract class AbstractArticleRepository extends EntityRepository
{
    ...
    public function getAdditionalTemplateParameters(ImageHelper $imageHelper, $context = '', $args = [])
    {
        ...
        if ($context == 'controllerAction') {
            ...
            $objectType = 'article';
            $thumbRuntimeOptions = [];
            $thumbRuntimeOptions[$objectType . 'Image'] = $imageHelper->getRuntimeOptions($objectType, 'image', $context, $args);
            ...
            if (in_array($args['action'], ['display', 'view'])) {
                ...
                $templateParameters['relationThumbRuntimeOptions'] = $imageHelper->getCustomRuntimeOptions('', '', 'PggoNewsDatesModule_relateditem', $context, $args);
            }
        }
    ...
    protected function getViewQuickNavParameters($context = '', $args = [])
    {
        ...
        $parameters['displayOnIndex'] = $this->getRequest()->query->get('displayOnIndex', '');
    ...
    public function truncateTable(LoggerInterface $logger)
    {
        ...
        $qb->delete('Pggo\NewsDatesModule\Entity\ArticleEntity', 'tbl');
        $query = $qb->getQuery();
    ...
    public function updateCreator($userId, $newUserId, TranslatorInterface $translator, LoggerInterface $logger, CurrentUserApi $currentUserApi)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('Pggo\NewsDatesModule\Entity\ArticleEntity', 'tbl')
        ...
        $logArgs = ['app' => 'PggoNewsDatesModule', 'user' => $currentUserApi->get('uname'), 'entities' => 'articles', 'userid' => $userId];
     ...
     *
     * @return array|articleEntity retrieved data array or articleEntity instance
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false)
     *
     * @return ArrayCollection collection containing retrieved articleEntity instances
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
    ...
    public function addCommonViewFilters(QueryBuilder $qb)
    {
        ...
        foreach ($parameters as $k => $v) {
            if ($k == 'catId') {
                 ...
                 */
                $categoryHelper = ServiceUtil::get('pggo_newsdates_module.category_helper');
                $qb = $categoryHelper->buildFilterClauses($qb, 'article', $v);
            } elseif (in_array($k, ['q', 'searchterm'])) {
                // quick search
                if (!empty($v)) {
                    $qb = $this->addSearchFilter($qb, $v);
                }
            } elseif (in_array($k, ['displayOnIndex'])) {
                // boolean filter
                if ($v == 'no') {
                    $qb->andWhere('tbl.' . $k . ' = 0');
                } elseif ($v == 'yes' || $v == '1') {
    ...
    protected function applyDefaultFilters(QueryBuilder $qb, $parameters = [])
    {
        ...
        $isAdminArea = false !== strpos($routeName, 'pggonewsdatesmodule_article_admin');
        if ($isAdminArea) {
            return $qb;
        }
    
        if (!in_array('workflowState', array_keys($parameters)) || empty($parameters['workflowState'])) {
            // per default we show approved articles only
    ...
    protected function addSearchFilter(QueryBuilder $qb, $fragment = '')
    {
        ...
        if (!$fragmentIsNumeric) {
            ...
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.teaser LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.bodyText LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $selection = 'COUNT(tbl.id) AS numArticles';
        if (true === $useJoins) {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('Pggo\NewsDatesModule\Entity\ArticleEntity', 'tbl');
     ...
     * @param int    $excludeId  Id of articles to exclude (optional)
     *
     * @return boolean result of this check, true if the given article does not already exist
     */
    ...
    protected function genericBaseQueryAddWhere(QueryBuilder $qb, $where = '')
    {
        if (!empty($where)) {
            ...
            $categoryHelper = ServiceUtil::get('pggo_newsdates_module.category_helper');
            $categoryProperties = $categoryHelper->getAllProperties('article');
            foreach ($categoryProperties as $propertyName => $registryId) {
                $config['plugins'][] = new CategoryFilter('PggoNewsDatesModule', $propertyName, 'categories' . ucfirst($propertyName));
            }
    ...
    /**
     * Update for articles becoming archived.
     *
    ...
    {
        if (true !== $session->get('PggoNewsDatesModuleAutomaticArchiving', false) && !$permissionApi->hasPermission('PggoNewsDatesModule', '.*', ACCESS_EDIT)) {
        ...
        foreach ($affectedEntities as $entity) {
            ...
            $url = new RouteUrl('pggonewsdatesmodule_article_display', $urlArgs);
