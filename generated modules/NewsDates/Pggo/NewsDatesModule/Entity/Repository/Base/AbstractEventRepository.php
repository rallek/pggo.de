 ...
 *
 * This is the base repository class for event entities.
 */
abstract class AbstractEventRepository extends EntityRepository
{
    ...
    public function truncateTable(LoggerInterface $logger)
    {
        ...
        $qb->delete('Pggo\NewsDatesModule\Entity\EventEntity', 'tbl');
        $query = $qb->getQuery();
    
        $query->execute();
    
        $logArgs = ['app' => 'PggoNewsDatesModule', 'entity' => 'event'];
    ...
    public function updateCreator($userId, $newUserId, TranslatorInterface $translator, LoggerInterface $logger, CurrentUserApi $currentUserApi)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('Pggo\NewsDatesModule\Entity\EventEntity', 'tbl')
        ...
        $logArgs = ['app' => 'PggoNewsDatesModule', 'user' => $currentUserApi->get('uname'), 'entities' => 'events', 'userid' => $userId];
     ...
     *
     * @return array|eventEntity retrieved data array or eventEntity instance
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false)
     *
     * @return ArrayCollection collection containing retrieved eventEntity instances
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
    ...
    public function addCommonViewFilters(QueryBuilder $qb)
    {
        ...
        foreach ($parameters as $k => $v) {
            if ($k == 'catId') {
                 ...
                 */
                $categoryHelper = ServiceUtil::get('pggo_newsdates_module.category_helper');
                $qb = $categoryHelper->buildFilterClauses($qb, 'event', $v);
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $selection = 'COUNT(tbl.id) AS numEvents';
        if (true === $useJoins) {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('Pggo\NewsDatesModule\Entity\EventEntity', 'tbl');
     ...
     * @param int    $excludeId  Id of events to exclude (optional)
     *
    ...
    protected function genericBaseQueryAddWhere(QueryBuilder $qb, $where = '')
    {
        if (!empty($where)) {
            ...
            $categoryHelper = ServiceUtil::get('pggo_newsdates_module.category_helper');
            $categoryProperties = $categoryHelper->getAllProperties('event');
